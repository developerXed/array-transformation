...
// 2. Split data into two arrays
function splitGroups(data) {
  const arr1 = []
  const arr2 = []

  data.forEach(item => (item.IdUser ? arr1 : arr2).push({ ...item }))

  return [arr1, arr2]
}

Object.entries(groups).forEach(([key, value]) => {
  groups[key] = splitGroups(value)
})

console.log(JSON.stringify(groups, null, 2)) // to log nested objects

// 3. Move obj from first array to second array if label contains user's input 
const labelInput = process.argv[2]?.toLowerCase()
Object.entries(groups).forEach(([key, value]) => {
  const results = []
  const arr1 = value[0].filter(item => {
    if (item.label?.toLowerCase().includes(labelInput)) {
      results.push({...item})
      return false
    } else {
      return true
  ...
  const arr2 = value[1].concat(results)

  groups[key] = [arr1, arr2]
})


// A good group to test it is with 'Appraisal' because it has data in both arrays
// console.log(groups['Appraisal'])
